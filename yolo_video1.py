...
sheet1.write(0, 0, 'count')
sheet1.write(0, 1, 'date/time')
sheet1.write(0, 2, 'class')
sheet1.write(0, 3, 'video date time ')
		...
		for i in idxs.flatten():
			...
			if (LABELS[classIDs[i]] in list_of_vehicles):
				...
				if (not boxInPreviousFrames(previous_frame_detections, (centerX, centerY, w, h), current_detections)):
					vehicle_count += 1
					sheet1.write(vehicle_count, 0, vehicle_count)
					sheet1.write(vehicle_count, 1, str(datetime.datetime.now()))
					text = "{}".format(LABELS[classIDs[i]])
					sheet1.write(vehicle_count, 2, text)
#					sheet1.write(vehicle_count, 3, str(datetime.hour))
...
#	start_time, num_frames = displayFPS(start_time, num_frames)
	...
	x = video_height
	y = video_width
	frame = frame[video_height//2:x, 0:y, :]
